...
# Here we implement the modified formula by replacing \bar{U} -> U0.
Ubar_in_formula = U0                   # used in Ps terms (steady-flow contribution)
...
def disc_sums(d, n, mask, Ubar_eff):
    if not np.any(mask):
        return 0.0, 0.0, 0.0
    k = d["k"][mask]
    chi = d["chi"][mask]
    ...
    w = d["omega"]
    Pt = w * chi * (psi ** 2) * sym
    Ps = (-Ubar_eff * k) * chi * (psi ** 2) * sym
    return float(Pt.sum()), float(Ps.sum()), float((Pt + Ps).sum())

def cont_sums(d, n, Ubar_eff):
    k = d["k_cont"]
    if k.size == 0:
        return 0.0, 0.0, 0.0
    psi = psi_hat(k, n)
...
def main(out="energy_conversion_rates_modified.pdf"):
    apply_style()
    ...
    ax[0].fill_between(
        kf, kf * U0, np.minimum(kf * U1, sigma_max),
        color="purple", alpha=0.22, zorder=0
    )
    ...
    dPs = fac * (-Ubar_in_formula * k) * R * (psi ** 2)
    dP = dPt + dPs
    ...
    # (c) grouped sums (n=0,1,2) using modified formula
    ...
    for n in (0, 1, 2):
        ...
        m = d["cg"] < 0
        if np.any(m):
            Pt, Ps, P = disc_sums(d, n, m, Ubar_in_formula)
        ...
        if d["k_cont"].size:
            Pt, Ps, P = cont_sums(d, n, Ubar_in_formula)
    ...
    ax[2].bar(x - wbar, Pt_all, wbar, label="Tide")
    ax[2].bar(x,        Ps_all, wbar, label="Steady")
    ax[2].bar(x + wbar, P_all,  wbar, label="Sum")
    for s in seps:
        if 0 < s < len(labels):
    ...
    fig.savefig(out)
    print(f"Saved: {out}")
    print(f"Modified formula used: Ubar_in_formula = U0 = {U0}")
